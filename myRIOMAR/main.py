...
import argparse
from myRIOMAR.TASKS import task
import matplotlib
...
def main():
    ...
    # === Call the corresponding task ===
    if args.command == 'download':
        task.download_data(task.core_arguments, overwrite=args.overwrite)
    elif args.command == 'plot':
        task.plot_global_maps(task.core_arguments, start_day=args.start_day, end_day=args.end_day)
    elif args.command == 'matchup':
        task.match_up_with_insitu(task.core_arguments, redo_the_MU_database=args.redo)
    elif args.command == 'maps':
        task.create_regional_maps(task.core_arguments, overwrite=args.overwrite)
    elif args.command == 'plumes':
        task.detect_plumes(task.core_arguments, use_dynamic_threshold=args.dynamic)
    elif args.command == 'timeseries':
        task.plot_timeseries_of_plume_surface(task.core_arguments)
    elif args.command == 'X11':
        task.X11_analysis_on_time_series(task.core_arguments)
    else:
        parser.print_help()
